...
from Fuzz4All.model import make_model, make_fix_model
from Fuzz4All.util.api_request import create_config, request_engine
from Fuzz4All.util.Logger import LEVEL, Logger


# mutate文件中写了多臂老虎机的具体实现，实现的初始化以及各种方法在类对象GA中
from Fuzz4All.util.mutate import GA
...
class Target(object):
    ...
    def validate_prompt(self, prompt: str):
        ...
        for fo in fos:
            ...
            # 将代码保存到/tmp/tempXXXXXX中，返回的是该文件的绝对路径
            ...
            if (
                # 代码能够正常运行
                result == FResult.SAFE
                # filter主要是匹配代码中是否有目标api
                and self.filter(code)
                # clean_code主要是清理代码中的由于大模型生成的评论，并且返回纯代码没有在集合中
                and self.clean_code(code) not in unique_set
            ):
                # 将纯净代码加入到集合中，并且将该提示词的得分加一
    ...
    def initialize(self):
        ...
        #init deepseek-coder
        #"""first-fuzzer
        self.fix_model, self.fix_tokenizer = make_fix_model()
        #"""
    ...
    def generate(self, **kwargs) -> Union[List[str], bool]:
        try:
            """
            self.generate_model()表示使用大模型进行代码的生成
            在batch_size=5的时候,fos里面就有5个代码
            """
    ...
    def update_strategy(self, new_code: str) -> str:
        ...
        while 1:
            ...
            elif strategy == 'mutate-existing':
                return f"\n{new_code}\n{self.m_prompt}\n"
            # semantically equivalent code generation
            elif strategy == 'semantic-equiv':
                return f"\n{new_code}\n{self.se_prompt}\n"
